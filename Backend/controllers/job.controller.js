...
export const getAllJobs = async (req, res) => {
  try {
    ...
    const jobs = await Job.find(query)
      ...
      .sort({ createdAt: -1 });//as we are posting job using company id, so when we are see in mongodb the company name is not showing, so we are using populate to show the company name in the job list.
